...
| `RAPPEL_NON_SERVI`         |    409 | réponse à une question de la banque non servie à ce participant           |
| `SEANCE_COMPLETE`          |    409 | capacité de la séance atteinte                                            |
| `VERSION_NON_PUBLIABLE`    |    409 | version illisible ou `verifierStructure` non vide au moment de la bascule |
...
| `PARTICIPANT_EVINCE`       |    401 | jeton d’un participant évincé                                             |
...
| `POST sessions/:id/answers` (existant)             | P      | 60                  | `{ questionId, valeur: number \| string, dureeMs }`                                                             | 201 `{ correcte, misconception, libelleConfusion }`                                                        | 400 `TYPE_DE_QUESTION` (production ou énigme) ; 409 `PHASE_FERMEE`, `RAPPEL_NON_SERVI`, `REPONSE_DEJA_ENREGISTREE` |
| `POST sessions/:id/free-responses` (existant)      | P      | 60                  | `{ screenId, activityId, response, dureeMs }`                                                                   | 201 `{ status: 'enregistre' }`                                                                             | 400 `ACTIVITE_INCONNUE`, texte vide                                                                                |
...
3. **Accès et capacité** : code erroné → refus, balayage protégé ; E1, E2, E3 rejoignent (graines
   ...
   évince X → 204, le jeton de X → 401 `PARTICIPANT_EVINCE`, la place est libérée ; F2 pilote S3 → 403 ;
...
19. **Rappels A6-05** : chaque poste reçoit R10, R11 et un ou deux rappels ; E2 (faux aux questions 2 et
    ...
    rechargement ; réponse à une question de la banque non servie → 409 `RAPPEL_NON_SERVI` ; carte de
...
| M02 | mineur   | banque et énigmes par `/answers` ; sélection recalculée           | intégré     | garde de type (400 `TYPE_DE_QUESTION`), sélection figée (`formation_rappels_servis`), 409 `RAPPEL_NON_SERVI`                                                                                                       |
