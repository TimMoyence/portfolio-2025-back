...
- `1789893879954-InsertB2CoursV3` insère **et publie** la version 3 du B2-01 (52 écrans, 38 remédiations, 5 médias) : le contenu inséré est le contenu servi, sans geste manuel après déploiement. Elle refuse le contenu que `lireCoursStocke` ou `verifierStructure` rejettent, republie la version 3 si elle est déjà en base, et son `down()` refuse de supprimer un contenu servi par une séance, republie la version précédente ou retire la ligne de publication quand il n'en reste aucune.
- `1789990000000-AlignB2CoursV3Samir` aligne la diapositive réglable de Samir avec le fil rouge du cours et raccourcit la consigne de la diapositive suivante ; la migration est réversible et réactive le déclencheur d'immutabilité après écriture.
- `1790100000000-PublierB2CoursEnrichi` insère **et publie** la version 3 du B2-01 construite par `src/migrations/data/b2-enrichi.cours.ts` à partir de la version 1, laissée intacte : l'écran `B2-01-A1-03-MISSION` passe en séance avec trois `questionsLibres` (une réponse libre par question, admise par `POST free-responses`), et l'écran `B2-01-A1-05-CORRECTION` (rendu v2 `sort-correction`) suit le tri `B2-01-A1-05-ANATOMIE` en rangeant chaque carte dans la catégorie du corrigé. Le numéro 3 est imposé par `contratPublicActif` : une version 1 ou 2 du B2 est lue comme contenu historique. Le `down()` refuse de retirer une version servie par une séance, sinon republie la version 1 et supprime la version 3.
...
- Postgres (`pnpm run test:integration:db:local`, qui monte puis arrête le Postgres de test) : `test/formations-{repositories,catalogue-b2,concurrence,resilience,seance}.db-integration.spec.ts`. Chaque suite ouvre `ouvrirContexteFormations()` (`test/helpers/formations-db.ts`), qui rejoue toute la chaîne des migrations formations, contenu B2 compris, et expose les vrais dépôts TypeORM, catalogue compris (`contexte.catalogue`).
- Séance complète sur la V3 migrée : `test/formations-e2e-seance-v3.db-integration.spec.ts` ouvre une séance de capacité 4 sur la version que la migration publie, déroule les 52 écrans et les 16 briques, éprouve `ECRAN_NON_SERVI`, `PHASE_FERMEE`, `PHASE_NON_MONOTONE`, `ENIGME_VERROUILLEE`, `TENTATIVES_EPUISEES`, `PRODUCTION_VIDE`, `SEANCE_COMPLETE`, `SEANCE_NON_DEMARREE`, `REPONSE_DEJA_ENREGISTREE` et `SEANCE_TERMINEE`, vérifie le flux `resultats` sur deux flux formateur, la reprise par `GET /moi`, la clôture, la rebascule du catalogue vers la v2 et l'absence de ligne orpheline après suppression de la séance. `test/formations-v3-classe.db-integration.spec.ts` joue la même V3 migrée pour une classe de trente, de l'ouverture à la clôture, dans les budgets d'AC-34.
