...
- `1790100000000-PublierB2CoursEnrichi` insère **et publie** la version 3 du B2-01 construite par `src/migrations/data/b2-enrichi.cours.ts` à partir de la version 1, laissée intacte : l'écran `B2-01-A1-03-MISSION` passe en séance avec trois `questionsLibres` (une réponse libre par question, admise par `POST free-responses`), et l'écran `B2-01-A1-05-CORRECTION` (rendu v2 `sort-review`) suit le tri `B2-01-A1-05-ANATOMIE` en rangeant chaque carte dans la catégorie du corrigé. Le numéro 3 est imposé par `contratPublicActif` : une version 1 ou 2 du B2 est lue comme contenu historique. Le `down()` refuse de retirer une version servie par une séance, sinon republie la version 1 et supprime la version 3.
