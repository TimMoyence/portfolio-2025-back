...
| `PATCH`  | `sessions/:id/control`                                 | Corps `{ ecran?, mode?, intervalle?, pilotage? }` ; `204`. Voir la borne d'écran et le pilotage d'écran ci-dessous.                                                                                            |
...
- **Déroulé** : il révèle les bonnes réponses du tirage de référence ; il n'est servi qu'au formateur propriétaire et aux administrateurs, jamais à un poste étudiant. Les `notes` sont des puces `• ` jointes par `\n` ; une note vide (`''`) signifie que l'écran n'en porte pas.
- **Diffusion** : un écran stocké sans `diffusion` est réservé à la séance (`DIFFUSION_PAR_DEFAUT = 'seance'`) et le catalogue public le sert verrouillé.
- **Écrans de correction** (`src/modules/formations/domain/cours/Corrections.ts`) : un écran `fp-story` `sort-review` ou `answer-review` corrige l'écran `props.source.screenId`, un `fp-worked` corrige l'écran `corrigeDe`. `verifierStructure` exige qu'il suive sa source et soit en diffusion `seance` (règle `correction-apres-source`). Le sujet porte `ecranCorrige` sur l'écran de correction et le sert **verrouillé** tant que `pilotage[source].revele` n'est pas vrai, quel que soit le rythme ; révélé, il porte `correction { ecranId, questions[] (bonne réponse du tirage du participant), corrige, reflexion }`. Séance terminée, toutes les corrections sont servies. Un écran peut aussi porter `renvoi`, l'écran dont afficher la miniature.
- **Pilotage d'écran** (`PATCH control`, corps `{ pilotage: { screenId, phase?, revele?, etayage?, reglages?, resultatsProjetes? } }`) : en rythme pilote, amener `ecranCourant` sur un écran de correction ou au-delà pose `revele: true` sur sa source dans la même mise à jour. `revele` et `etayageAtteint` (maximum de `etayage`, calculé par le serveur, refusé en entrée) sont monotones : un retour arrière ne rouvre rien. `revele` ferme toutes les réponses de l'écran (questions, productions, réponses libres) en `409` `PHASE_FERMEE`, de même qu'un `etayageAtteint` positif. `reglages` est admis sur `fp-concept4` et `fp-plot`, `etayage` sur `fp-worked`, `fp-sheet` et `fp-table-build`.
- **Reprise d'une production** : tant que l'écran n'est ni révélé ni étayé, un second envoi sur `fp-sheet` ou `fp-table-build` remplace la production et rend le nouveau verdict ; la maîtrise n'est comptée qu'au premier envoi. Les envois sont plafonnés à trois par production (`SOUMISSIONS_MAX_PAR_PRODUCTION`, colonne `soumissions` incrémentée par un `UPDATE` conditionnel, donc sûr face aux envois concurrents) : au-delà, `409` `REPRISES_EPUISEES` et la dernière version reste enregistrée. Le tri de cartes (`fp-cardsort`) reste à envoi unique (`409` `REPONSE_DEJA_ENREGISTREE`). Chaque réponse libre enregistrée pousse l'événement `resultats` au flux formateur.
...
- Les tables des séances viennent de `1778900000000-CreateFormations`, puis de `1780300000000` à `1780600000000` (réponses libres, annotations, groupes aujourd'hui inutilisés, scores). Les tables du contenu versionné, leurs contraintes et le déclencheur d'immutabilité viennent de `1779100000000` à `1780100000000`. Aucune migration n'insère de contenu de cours. `1789818127042-DropFormationScreenNotesDefault` retire le défaut vide de `formation_screen_contents.notes`.
- `1790300000000-NotesFormateurFacultatives` remplace le CHECK `chk_formation_screen_notes_not_blank` par `chk_formation_screen_notes_absentes_ou_renseignees` (`"notes" = '' OR "notes" ~ '[^[:space:]]'`) : un écran peut n'avoir aucune note, jamais une note faite de blancs, retours à la ligne compris. Le retour arrière repose l'ancien CHECK en `NOT VALID`, pour ne pas échouer sur les écrans déjà publiés sans note.
