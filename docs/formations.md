...
- Les cours publies vivent dans `src/modules/formations/domain/cours/catalogue/` : le referentiel B2 et ses douze sequences sont declares dans `src/modules/formations/domain/cours/referentiel-b2.ts`; les cours effectivement publies sont inscrits dans `COURS_PUBLIES` (`index.ts`) qui alimente le catalogue `CATALOGUE_COURS_STATIQUE`.
...
`pnpm --silent cours:exporter-sujet <slug> <graine>` (`scripts/exporter-sujet.ts`) ecrit sur la sortie standard le sujet JSON d'un cours publie pour une graine de `[0, 2 147 483 647)`, et rien d'autre du tirage : ni solutions, ni corriges. Il sert a produire la fixture du front (`src/testing/fixtures/cours/b2-01-traitement-information-chiffree.sujet.json`, graine `42`). Un slug inconnu, une graine invalide ou ambigue sortent en erreur avec l'usage.

## Tests

- Contrats HTTP et SSE : `test/formations-session.http-socket.spec.ts` (`pnpm run test:e2e:http`).
- Postgres : `test/formations-{repositories,concurrence,resilience,seance,repetition-b2-01}.db-integration.spec.ts` (`pnpm run test:integration:db:local`, qui monte puis arrete le Postgres de test).
- Repetition a blanc : `test/formations-repetition-b2-01.db-integration.spec.ts` joue la seance du vrai B2-01 (`CATALOGUE_COURS_STATIQUE`) de l'ouverture a la cloture contre Postgres : trente etudiants inscrits, sujets lus sans aucune cle du corrige, chaque ecran pilote, chaque question repondue avec des pieges, resultats pousses au flux formateur puis relus par `GET results`.
...
- Charge : `test/formations-charge.db-integration.spec.ts`, exclue de `test:integration:db` (`test/jest-db-integration.json`) et lancee a part par `pnpm run db:integration:up`, `pnpm run test:charge:run`, puis `pnpm run db:integration:down`. Elle mesure le vrai B2-01 (`CATALOGUE_COURS_STATIQUE`, 9 questions). Budgets : trente inscriptions simultanees en 5 s ; trente reponses simultanees en 3 s, flux du formateur ouvert et recalculant les resultats jusqu'a les avoir pousses ; cloture de 1 170 reponses en 10 s ; lecture des resultats en 2 s ; trente flux etudiants tenus deux minutes avec moins de 20 % de croissance memoire.
