...
export function declaredDependencies(manifest) {
  ...
  return [...new Set(names.flatMap(anchorNamesOf))]
    .filter(
      (name) =>
        name.length >= DEPENDENCY_MIN_LENGTH &&
        !GENERIC_PACKAGE_NAMES.has(name),
    )
    .sort();
}
...
function typedJsdocIndexes(contents) {
  ...
  return proseIndexes(contents).filter(
    (i) => i < tagAt || isForeignTag(contents[i]),
  );
}
 ...
 */
export function alwaysOffending(
  { raw, startLine },
  { jsdocTypes = false } = {},
) {
