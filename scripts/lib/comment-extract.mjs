...
function emptyBraceRanges(sf) {
  ...
  const walk = (node) => {
    if (isEmptyBraces(node))
      ranges.push({ from: node.getStart(sf), to: node.getEnd() });
...
function isContiguous(previous, next) {
  ...
  return (
    next.startLine === previous.endLine + 1 && next.column === previous.column
  );
}
...
export function mergeContiguous(blocks) {
  const ordered = [...blocks].sort(
    (a, b) => a.startLine - b.startLine || a.column - b.column,
  );
  /** @type {ExtractedBlock[]} */
  const merged = [];
