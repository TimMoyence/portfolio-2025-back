...
describeDb('Enigmes du mini-jeu (B7, db integration)', () => {
  ...
  it('ne compte que dix tentatives quand vingt partent en parallele', async () => {
    ...
    expect(reponses.filter((reponse) => reponse.status === CREE)).toHaveLength(
      TENTATIVES_MAX_DE_TEST,
    );
  ...
  it('refuse la onzieme tentative en 409 TENTATIVES_EPUISEES', async () => {
    ...
    for (let rang = 0; rang < TENTATIVES_MAX_DE_TEST; rang += 1) {
      await tenter(seance, ENIGMES_DE_TEST[0], String(200 + rang)).expect(CREE);
    }
  ...
  it('refuse le jeton d un participant d une autre seance', async () => {
    const seance = await ouvrirSeance('55555555-5555-4555-8555-000000000009');
    const autre = await ouvrirSeance('55555555-5555-4555-8555-000000000010');
